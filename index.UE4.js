...
var Module = {
	...
	totalDependencies: 0,
	dataBytesStoredInIndexedDB: 5000000000, // Track how much data is currently stored in IndexedDB.
	...
	UE4_indexedDBVersion: 202108190145, // Bump this number to invalidate existing IDB storages in browsers.
};
...
function heuristicIs64Bit(type) {
	...
	var ua = (navigator.userAgentData + ' ' + navigator.oscpu).toLowerCase();
...
$(document).ready(function() {
	...
	if (!supportsWasm) {
		showErrorDialog('Your browser does not support WebAssembly. Please try updating to latest 64-bit browser that supports WebAssembly.<br>Current user agent: ' + navigator.userAgentData);
		return;
	}
...
//	}
	...
	function shouldBrowserSupportWebGL2() {
		var match = window.navigator.userAgentData.match(/Firefox\/([0-9]+)\./);
